############ This script processes E9.5 Endodoerm Cells using Seurat [v2.3.4]###############
...
M <- CreateSeuratObject(raw.data = data, min.cells =3, min.genes = 100, project= 'Endoderm_E9.5')
######################################################################
...
dev.off()
M <- FilterCells(object = M, subset.names = c("nGene", "percent.mito"), low.thresholds = c(-Inf, -Inf), high.thresholds = c(6000, 1))
...
######################### Save Robject ###############################################################
save(M, file = 'Seurat_v2.3.4_AnalysisOf_E9.5_EndodermCells.Robj')
######################################################################################################