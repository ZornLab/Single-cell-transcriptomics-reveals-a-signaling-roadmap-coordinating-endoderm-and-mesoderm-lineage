############ This script processes E8.5 Endodoerm Cells using Seurat [v2.3.4]###############
...
############ Please note blood, ribosomal, mitochondrial and certain strain-specific ncRNA's were regressed out before Seurat analysis
...
M <- CreateSeuratObject(raw.data = data, min.cells =3, min.genes = 100, project= 'Endoderm_E8.5')
######################################################################
...
dev.off()
M <- FilterCells(object = M, subset.names = c("nGene", "percent.mito"), low.thresholds = c(-Inf, -Inf), high.thresholds = c(5000, 1))
...
######################### Save Robject ###############################################################
save(M, file = 'Seurat_v2.3.4_AnalysisOf_E8.5_EndodermCells.Robj')
######################################################################################################